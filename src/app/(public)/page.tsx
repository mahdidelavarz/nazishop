...
//     // Logout from Zustand store (this will clear guest cart via authStore)
//     logout();
    
//     // Also logout from Supabase if Google OAuth was used
//     const { supabase } = await import('@/shared/lib/supabase');
...
//                     {userName || userEmail || phoneNumber || "کاربر"}
//                   </span>
//                 </div>
...
export default function HomePage() {
  ...
  const handleLogout = async () => {
    logout() // handled by React Query mutation inside useAuth
  }
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-4">
                <div className="hidden md:flex items-center gap-2 bg-gray-50 px-4 py-2 rounded-full">
                  <Icon icon="ph:user-circle-duotone" width={24} className="text-gray-600" />
                  <span className="text-sm font-medium text-gray-700">
                    {user?.fullName || user?.email || user?.phoneNumber || "کاربر"}
                  </span>
                </div>
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-red-50 text-red-600 hover:bg-red-100 px-4 py-2 rounded-lg transition disabled:opacity-50"
                >
                  <Icon icon="ph:sign-out-duotone" width={20} />
                  <span className="hidden sm:inline">خروج</span>
                </button>
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6">
          {isAuthenticated && user?.fullName && (
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-pink-100 to-purple-100 px-6 py-3 rounded-full mb-4">
              <Icon icon="ph:hand-waving-duotone" className="text-2xl" />
              <span className="font-medium text-gray-800">
                سلام {user.fullName} عزیز، خوش آمدید!
              </span>
            </div>
          )}
          ...
          {[
            ...
            >
              <div className="w-16 h-16 bg-gradient-to-br from-pink-100 to-purple-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <Icon icon={f.icon} className="text-pink-600" width={32} />
              </div>
